...
const contact = () => {
 ...
 console.log(user, 'testing')
  ...
  const barns = () => {
    axios
      .get(`http://localhost:5000/get-user-barn/${getuserbarn} `)
      .then((res) => {
  ...
  const feedLenght = () => {
    axios
      .get(`http://localhost:5000/get-feed-bag/${user?.userId}`)
      .then((res) => {
        const data = res.data;
 ...
 const checkandclaim = async (fishPurchaseId, userId)=>{
  axios.get(`http://localhost:5000/get-earn-data/${fishPurchaseId}`).then((res)=>{
      if(res.status == 400){
        setModalHandle('flex')
        setModalTitle(res.data.error);
        setM2(true);
